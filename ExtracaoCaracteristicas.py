...
""" Contraste, dissimilaridade, homogeneidade, asm, energia, correlacao,
    area, perimetro, diametro, redondeza, compactness, mediaRGB,
    desvioPadraoRGB, varianciaRGB, curtoseRGB, entropiaRGB, mediaHSV,
    desvioPadraoHSV, curtoseHSV, entropiaHSV """

class ExtracaoCaracteristicas:

    """ Construtor """
    def __init__(self,lesoes):
        self.lesoes = lesoes
        self.atributosCor = None
        self.atributosForma = None
        self.atributosTextura = None
        self.atributos = None

    """ Metodo prove todos os atributos juntos """
    def extracaoAtributos(self):
        self.extracaoCor()
        #self.extracaoForma()
        self.extracaoTextura()

        #atributos = np.concatenate((
        #    self.atributosTextura,self.atributosForma),axis=1)

        atributos = self.atributosTextura

        for i in xrange(len(self.atributosCor)):
            atributos = np.concatenate((
                atributos,self.atributosCor[i, :]),axis=1)

        self.atributos = atributos

    def saveCSV(self):
        np.savetxt("data.csv", self.atributos, delimiter=";")

    """ Metodo extrai atributos de forma """
    def extracaoForma(self):
        ...
        for i in xrange(len(self.lesoes)):

            # Converte para escala de cinza e encontra a mascara da lesao.
            arrayLesoesGrey.append(cv2.cvtColor(self.lesoes[i], cv2.COLOR_RGB2GRAY))
        ...
        self.atributosForma = np.asarray(atributosForma).transpose()

    """ Metodo extrai atributos de textura """
    def extracaoTextura(self):
        ...
        self.atributosTextura = np.asarray(atributosTextura).transpose()

    """ Metodo extrai atributos de cor """
    def extracaoCor(self):
        #print("Extraindo cor...")

        atributosCor = []

        nElementos = len(self.lesoes)
        arrayLesoesGrey = []
        arrayLesoesHSV = []
        ...
        for i in xrange(nElementos):
            ...
            # Converte para HSV
            arrayLesoesHSV.append(cv2.cvtColor(self.lesoes[i], cv2.COLOR_RGB2HSV))
            ...
            for j in xrange(3):
                ...
                #plt.show()
                histograma = cv2.calcHist([self.lesoes[i]], channels=[j], mask=maskb, histSize=[256], ranges=[0,256])
                ...
                (mediaRGB[i][j], desvioPadraoRGB[i][j]) = cv2.meanStdDev(self.lesoes[i][:, :, j], mask=np.uint8(maskb))
        ...
        self.atributosCor = np.asarray(atributosCor)